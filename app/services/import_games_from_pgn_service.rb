class ImportGamesFromPGNService
  def initialize(pgn_file_path, user, collection_name = nil)
    @pgn_file_path = pgn_file_path
    @collection = Collection.create!(name: collection_name, user: user) unless collection_name.nil?
  end
  ...
  def call
    ...
    pgn_games.each do |pgn_game|
      ...
      begin
        if pgn_game.tags["White"].split(",").count >= 2
          ...
          white_player_last_name = pgn_game.tags["White"]
        end

        if pgn_game.tags["Black"].split(",").count >= 2
          black_player_first_name = pgn_game.tags["Black"].split(",")[1].strip
          black_player_last_name = pgn_game.tags["Black"].split(",")[0].strip
        else
          black_player_first_name = ""
          black_player_last_name = pgn_game.tags["Black"]
        end
        ...
        p "creating tags"
        Tag.create!(game: game, collection: @collection) if @collection
